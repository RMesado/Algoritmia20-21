...
def recursos_rec(v: Dict[Tuple[int, int], int], m: List[int], U: int) -> int:
    def B(n: int, u: int) -> int:
        if n == 0:
            ...
            # return max( [B(n - 1, u - k) + v[n - 1, k] for k in range(min(m[n - 1], u) + 1)]) #Version top
            sol = -infinity
            for k in range(min(m[n - 1], u) + 1):
                sol = max(sol, B(n - 1, u - k) + v[n - 1, k])
            return sol
...
def recursos_iter_camino(v: Dict[Tuple[int, int], int], m: List[int], U: int) -> Tuple[int, List[int]]:
    ...
    for n in range(1, N + 1):
        for u in range(U + 1):
            ...
            mem[n, u] = -infinity, ()
            for k in range(min(m[n - 1], u) + 1):
                mem[n, u] = max(mem[n, u], (mem[n - 1, u - k][0] + v[n - 1, k], (n - 1, u - k, k)))
    # --------------------
    score = mem[N, U][0]
    sol = []
    n, u = N, U
...
def recursos_iter_reduccion_coste(v: Dict[Tuple[int, int], int], m: List[int], U: int) -> int:
    ...
    for n in range(1, N + 1):
        ...
        for u in range(U + 1):
            ...
            current[u] = -infinity
            for k in range(min(m[n - 1], u) + 1):
                current[u] = max(current[u], previous[u - k]+ v[n - 1, k])
                # --------------------
    return current[U]
